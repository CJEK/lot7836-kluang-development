...
const styles = [
  {
    ...
    images: [
      { src: 'assets/style2-ext.jpg?v=21', label: ['1/4 正面 50ft 外观全景 (落座 150mm 地坪)', '1/4 Front Exterior 3D'] },
      { src: 'assets/style2-side.jpg?v=21', label: ['2/4 75ft 侧立面与服务排水 3D 图', '2/4 75ft Side Facade 3D'] },
      { src: 'assets/style2-rear.jpg?v=21', label: ['3/4 45° 轴测后方全景与拔风塔 3D 图', '3/4 45° Rear Isometric 3D'] },
      { src: 'assets/style2-int.jpg?v=21', label: ['4/4 室内花砖日光中庭 3D 图', '4/4 Interior Atrium 3D'] }
    ]
  },
  {
    key: 'style1',
    ...
    images: [
      { src: 'assets/style1-ext.jpg?v=21', label: ['1/4 正面 50ft 外观全景 (落座 150mm 地坪)', '1/4 Front Exterior 3D'] },
      { src: 'assets/style1-side.jpg?v=21', label: ['2/4 75ft 侧立面 3D 图', '2/4 75ft Side Facade 3D'] },
      { src: 'assets/style1-rear.jpg?v=21', label: ['3/4 45° 轴测后方全景与拔风塔 3D 图', '3/4 45° Rear Isometric 3D'] },
      { src: 'assets/style1-int.jpg?v=21', label: ['4/4 室内挑高大厅 3D 图', '4/4 Interior Hall 3D'] }
    ]
  },
  {
    key: 'style3',
    ...
    images: [
      { src: 'assets/style3-ext.jpg?v=21', label: ['1/4 正面 50ft 外观全景 (落座 150mm 地坪)', '1/4 Front Exterior 3D'] },
      { src: 'assets/style3-side.jpg?v=21', label: ['2/4 75ft 侧立面 3D 图', '2/4 75ft Side Facade 3D'] },
      { src: 'assets/style3-rear.jpg?v=21', label: ['3/4 45° 轴测后方全景与拔风塔 3D 图', '3/4 45° Rear Isometric 3D'] },
      { src: 'assets/style3-int.jpg?v=21', label: ['4/4 室内纯白大厅 3D 图', '4/4 Interior White Hall 3D'] }
    ]
  }
];

const drawings = [
  ['A-101', ['DWG A-101 2D 建筑平面图', 'DWG A-101 Floor Plan'], ['50x75ft 地块、双 40ft HC 集装箱、34ft 挑高大厅与后区 35ft 阁楼平面布局。', '50x75ft footprint, twin 40ft HC containers, 34ft hall and rear loft.'], 'assets/floorplan-bp.jpg?v=21'],
  ['A-102', ['DWG A-102 建筑正立面图', 'DWG A-102 Elevation'], ['控制标高：+24ft 主屋檐、+38ft-5in 主屋脊与 +47ft 拔风塔顶，30° 坡屋顶结构。', 'Level controls: +24ft eave, +38ft-5in ridge and +47ft Jack Roof.'], 'assets/elevation-bp.jpg?v=21'],
  ['S-101', ['DWG S-101 集装箱切割加固 Detail', 'DWG S-101 Container Detail'], ['侧墙切割开窗、切口周圈 100x100x4.5mm RHS 方钢框焊接与 M20 锚栓加固。', '100x100x4.5mm RHS steel frame reinforcement & M20 anchor bolts.'], 'assets/container_splicing_blueprint.jpg?v=21'],
  ['M-101', ['DWG M-101 厨房与集中排水图', 'DWG M-101 Kitchen & Drainage'], ['左集装箱 25ft 厨房、50L 油脂拦截器、DN100 黑水管(1:40坡度)及 8PE 化粪池。', 'Kitchen grease trap, DN100 blackwater pipe (1:40 slope) & 8PE septic tank.'], 'assets/kitchen_drainage_blueprint.jpg?v=21'],
  ['F-101', ['DWG F-101 BOMBA 消防逃生图', 'DWG F-101 Fire & Egress Plan'], ['SD1-SD6 烟感、FE1-FE4 灭火器、1.5m 门净宽、Loft 第二逃生梯及疏散路线。', 'SD1-SD6 detectors, FE1-FE4 extinguishers, 1.5m exit doors & 2nd escape stair.'], 'assets/fire_safety_egress_blueprint.jpg?v=21'],
  ['T-101', ['DWG T-101 5 大工程打压测试图', 'DWG T-101 T&C Testing Plan'], ['8 Bar 给水打压、24h 湿区闭水、4h 屋顶高压喷淋及 30mA RCCB 漏电测试。', '8 Bar water pressure, 24h flood test, 4h roof spray & 30mA RCCB tests.'], 'assets/tc_testing_blueprint.jpg?v=21'],
  ['G-101', ['DWG G-101 总平面规划图', 'DWG G-101 Site Plan'], ['Jalan Pakis 正面沿街、指北针、40x50ft 建筑占地、15ft 前退缩、10ft 后退缩及 U 型排水沟。', 'Jalan Pakis frontage, 40x50ft footprint, 15ft front & 10ft rear setbacks.'], 'assets/siteplan-bp.jpg?v=21']
];
...
function renderCards() {
  ...
  if (drawingGrid) {
    drawingGrid.innerHTML = drawings.map(([number, title, description, hdSrc]) => `
      <article class="drawing-card image-button" data-image="${hdSrc || `assets/drawings/${number.toLowerCase()}-preview.png?v=21`}" data-caption="${number} ${title[isZh ? 0 : 1]}" style="cursor:pointer; background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding:16px; transition: transform 0.2s, box-shadow 0.2s;">
        <div style="position:relative; aspect-ratio:1.42; overflow:hidden; border-radius:8px; margin-bottom:12px; border:1px solid #cbd5e1;">
          <img src="assets/drawings/${number.toLowerCase()}-preview.png?v=21" alt="${number}" style="width:100%; height:100%; object-fit:cover; display:block;" loading="lazy">
